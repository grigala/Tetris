...
public class MainMap extends TileView {
     ...
     * - mapOld - hold the map without current Tetris
     * - mapLast - hold the map before last move of Tetris
     */
    private static TetrisMap mapCur;
    public static TetrisMap mapOld;
    private static TetrisMap mapLast;

    private static int[] randArr = {-1,-1};
    /**
     *  This parameter is the flag that indicate that Action_Down event
     *  was occur and Tetris was moved left or right
     */
        ...
        public void handleMessage(Message msg) {
            if(mGameState == READY) {
                if(msg.what == 1) {//TODO change to final Name
                    ...
                    curTetris = newTetris(getRandomFromArr(), 4, 0);//TODO check this
                    Log.d(TAG,"Next: " + Integer.toString(randArr[1]));
                    tempCount++;
                    if(!mapCur.putTetrisOnMap(curTetris)) {
                        Log.d(TAG, "Game Over!");
                        initNewGame();
                        mGameState = PAUSE;
                    }
                    ...
                    gameMove();//TODO insert this function to the Tetris
    ...
    private void initMainMap() {
        setFocusable(true);
        mapCur = new TetrisMap();
        mapOld = new TetrisMap();
        mapLast = new TetrisMap();
    ...
    private Tetris newTetris(int type, int x, int y) {
        switch(type){
            case L_TYPE:
                return new LTetris(x, y);
            case J_TYPE:
                return new JTetris(x, y);
            case T_TYPE:
                return new TTetris(x, y);
            case Z_TYPE:
                return new ZTetris(x, y);
            case S_TYPE:
                return new STetris(x, y);
            case O_TYPE:
                return new OTetris(x, y);
            case I_TYPE:
                return new ITetris(x, y);
            default:
                return new LTetris(x, y);
     ...
     */
    private int[] coordArrayListToArray(TetrisMap map) {
        int[] rawArray = new int[TetrisMap.MAP_X_SIZE*TetrisMap.MAP_Y_SIZE];
        for (int row = 0; row < TetrisMap.MAP_Y_SIZE; row++) {
            for (int col = 0; col < TetrisMap.MAP_X_SIZE; col++) {
                rawArray[row*TetrisMap.MAP_X_SIZE+col] = map.getMapValue(col, row);
            }
        }
        return rawArray;
    }
     ...
     */
    private TetrisMap coordArrayToArrayList(int[] rawArray) {
        TetrisMap tMap = new TetrisMap();//TODO change to get map from argument
        int arrSize = rawArray.length;
        for (int i = 0; i < arrSize; i++) {
            tMap.setMapValue(i%TetrisMap.MAP_X_SIZE,(i/TetrisMap.MAP_Y_SIZE),rawArray[i]);
        }
        return tMap;
    }
            ...
            try
            {
                ...
                if(event.getAction() == MotionEvent.ACTION_MOVE && mGameState == READY && !pausePressed) {
                    ...
                    if ((xInitRaw - xCurRaw) > xMoveSens && Math.abs(yInitRaw - yCurRaw) < dropSensativity) {
                        ...
                        mapCur.copyFrom(mapOld);
                        for (int i = 0; i < q; i++) {
                            if (curTetris.moveLeft(mapCur) &&
                                    !curTetris.isColusionY(curTetris.getYPos()+1, curTetris.getXPos(), curTetris.sMap, mapCur, false)) {
                            ...
                            mapCur.putTetrisOnMap(curTetris);
                        }
                        update();
                    }
                ...
                if(event.getAction() == MotionEvent.ACTION_UP)
                {
                    ...
                    if(mGameState == READY && !pausePressed){
                        ...
                        if(yCurRaw - yInitDrop > dropSensativity && timeDelta < deltaTh) {
                            mapCur.resetMap();
                            mapCur.copyFrom(mapOld);
                            curTetris.drop(mapCur);
                            mapCur.putTetrisOnMap(curTetris);
                            update();
                            mRedrawHandler.removeMessages(0);
                            mRedrawHandler.sendEmptyMessage(1);//TODO change to final name
                        }
                        //Rotate Tetris (release on same x pos)
                        else if (!wasMoved && Math.abs(yCurRaw - yInitRaw) < rotateSens ) {
                            mapCur.resetMap();
                            mapCur.copyFrom(mapOld);
                            curTetris.rotateTetris(mapCur);
                            mapCur.putTetrisOnMap(curTetris);
                            update();
                        }
                    }
