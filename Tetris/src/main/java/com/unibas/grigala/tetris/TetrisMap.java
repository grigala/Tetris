...
public class TetrisMap {
    ...
    /**
     * This function puts the Tetris on the map
     * and check collision with the other objects on a map
     * @param shape - Tetris to put
     * @return true if puts successful otherwise false
     */
    public boolean putTetrisOnMap(Tetris shape) {
        for(int col = 0; col < shape.getSize(); col++){
            for(int row = 0; row < shape.getSize(); row++) {
                if (shape.sMap[col][row] != TileView.BLOCK_EMPTY) {
                    if(shape.getXPos() + col >= 0 && shape.getXPos() + col < TetrisMap.MAP_X_SIZE &&
                            shape.getYPos() + row >= 0 && shape.getYPos() + row < TetrisMap.MAP_Y_SIZE &&
                ...
                if(Tetris.ghostEnabled && !shape.onGhost()) {
    ...
    public void copyFrom(TetrisMap srcMap) {
        for (int x = 0; x < TetrisMap.MAP_X_SIZE; x++) {
            for(int y = 0; y < TetrisMap.MAP_Y_SIZE; y++) {
